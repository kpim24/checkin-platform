...
const MAX_RESTART_COUNT = process.env.MAX_RESTART_COUNT != null && process.env.MAX_RESTART_COUNT !== '' ? parseInt(process.env.MAX_RESTART_COUNT, 10) : Infinity;
...
function killOrphansByPort(port) {
  try {
    ...
    if (pids) {
      ...
      for (const p of pidList) { try { process.kill(parseInt(p, 10), 'SIGKILL'); } catch {} }
      return pidList;
    }
  } catch {}
  return [];
...
function startProcess({ name, command, args, cleanupPort }) {
  ...
  const run = async () => {
    let restartCount = 0;
    while (!stopping) {
      const child = spawn(command, args, { detached: true, stdio: ['ignore', 'pipe', 'pipe'], shell: true, cwd: PROJECT_ROOT, env: { ...process.env } });
      entry.pid = child.pid;
      entry.child = child;
      ...
      if (entry.pid) { killProcessGroup(entry.pid, 'SIGTERM'); await sleep(2000); killProcessGroup(entry.pid, 'SIGKILL'); }
      entry.pid = null; entry.child = null;

      if (cleanupPort) {
        const orphans = killOrphansByPort(cleanupPort);
        if (orphans.length > 0) { logEvent('WARN', name, `Killed orphan processes on port ${cleanupPort}: ${orphans.join(' ')}`); await sleep(500); }
      }

      if (stopping) break;
      const runDuration = (Date.now() - startTime) / 1000;
      if (runDuration >= 60) { restartCount = 0; logEvent('INFO', name, `Ran for ${Math.round(runDuration)}s, resetting restart counter`); }
      else { restartCount++; }
      if (restartCount >= MAX_RESTART_COUNT) { logEvent('ERROR', name, `Max restart count (${MAX_RESTART_COUNT}) reached, giving up`); break; }
...
async function cleanup() {
  ...
  cleanupDone = true; stopping = true;
  logEvent('INFO', 'main', 'Shutting down all processes...');
  for (const entry of managedProcesses) { if (entry.pid) { logEvent('INFO', 'main', `Stopping process group (PGID: ${entry.pid})`); killProcessGroup(entry.pid, 'SIGTERM'); } }
  await sleep(2000);
  for (const entry of managedProcesses) { if (entry.pid) { logEvent('WARN', 'main', `Force killing process group (PGID: ${entry.pid})`); killProcessGroup(entry.pid, 'SIGKILL'); } }
  killOrphansByPort(SERVER_PORT); killOrphansByPort(CLIENT_DEV_PORT);
...
function cleanStaleDist() {
  ...
  if (fs.existsSync(distPath)) { fs.rmSync(distPath, { recursive: true, force: true }); logEvent('INFO', 'main', 'Cleaned dist/ to force full rebuild'); }
}
...
async function main() {
  ...
  try { execSync('fullstack-cli action-plugin init', { cwd: PROJECT_ROOT, stdio: 'inherit' }); writeOutput('✅ Action plugins initialized\n\n'); } catch { writeOutput('⚠️  Action plugin initialization failed, continuing anyway...\n\n'); }
  const serverPromise = startProcess({ name: 'server', command: 'npm', args: ['run', 'dev:server'], cleanupPort: SERVER_PORT });
  const clientPromise = startProcess({ name: 'client', command: 'npm', args: ['run', 'dev:client'], cleanupPort: CLIENT_DEV_PORT });
